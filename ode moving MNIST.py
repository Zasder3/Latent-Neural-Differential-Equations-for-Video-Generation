...
def genSamples(g, n=8, e=1):
    ...
    io.vwrite(
        f'video_samples/moving_mnist/odemoving_2/gensamples_id_big{e}.gif',
        out
    )
...
def train():
    ...
    dg = dataGen()
    # gen model
    dis = DisMultiResNet(channels=[32, 64, 128, 256, 512], colors=1).cuda()
    gen = Generator_ODE(
        tempc=512,
        zt_dim=4,
        upchannels=[256],
                ...
                torch.save(
                    {'epoch': epoch,
                     ...
                     'optimizer_state_dict': [genOpt.state_dict(),
                                              disOpt.state_dict()]},
                     f'checkpoints/moving_mnist/ode_s2_checkpoints/state_big{epoch}.ckpt')

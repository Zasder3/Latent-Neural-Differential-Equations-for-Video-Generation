...
def genSamples(g, n=8, e=1):
    ...
    io.vwrite(
        f'video_samples/moving_mnist/tganv2moving/gensamples_id{e}.gif',
        out
    )
...
def train():
    ...
    # resume training
    state_dicts = torch.load(f'tganv2_checkpoints/state9000.ckpt')
            ...
            if epoch % 1000 == 0 or epoch == epochs-1:
                torch.save({'epoch': epoch,
                            ...
                            'optimizer_state_dict': [genOpt.state_dict(),
                                                     disOpt.state_dict()]},
                           f'tganv2_checkpoints/state{epoch}.ckpt')
    torch.save({'epoch': epoch,
                'model_state_dict': [gen.state_dict(),
                                     dis.state_dict()],
                'optimizer_state_dict': [genOpt.state_dict(),

import torch
import os
import numpy as np
from models.tgan import VideoGenerator
from evaluation_metrics import calculate_inception_score
from tqdm.gui import tqdm

path = 'UCF101/tgan'

if __name__ == '__main__':
    # gen model
    gen = VideoGenerator()

    scores = []
    iters = []
    best = 0
    best_name = None
    for name in tqdm(os.listdir(f'checkpoints/{path}/')):
        state_dicts = torch.load(f'checkpoints/{path}/{name}')

        gen.load_state_dict(state_dicts['model_state_dict'][0])
        gen.cpu()
        scores.append(calculate_inception_score(gen, test=False, zdim=100))
        iters.append(name[12:-5])
        if scores[-1] > best:
            best = scores[-1]
            best_name = name
        print(iters[-1], scores[-1])
    print(best_name)
    np.save('tganv2_inception.npy', [iters, scores])
